...
int main(void)
{
	...
	// create the kernel
	const char *Second_kernel_name = "InputKernel";
...
//	assert(err==CL_SUCCESS);
	cl::Kernel Second_kernel(program, Second_kernel_name, &err);
	assert(err==CL_SUCCESS);
	...
	//////////////     Set Arguments to the Kernels
	//////////////       Exercise 2   Step 2.6    ///////////////////
	err = Second_kernel.setArg(0, Buffer_In2);
	assert(err==CL_SUCCESS);
	err = Second_kernel.setArg(1, vectorSize);
	assert(err==CL_SUCCESS);
...
//	err=myqueue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(vectorSize), cl::NDRange(workSize));
	...
	err=myqueue.enqueueTask(Second_kernel);
//	assert(err==CL_SUCCESS);
