...
int main(void)
{
	...
	cl::CommandQueue myqueue_2(mycontext, DeviceList[0]);
	assert(err==CL_SUCCESS);
	...
	err = myqueue_2.enqueueWriteBuffer(Buffer_In2, CL_FALSE, 0, sizeof(cl_float)*vectorSize, Y);
	assert(err==CL_SUCCESS);
	myqueue.finish();
	...
	cl::Kernel kernel_2(program, "InputKernel", &err);
	assert(err==CL_SUCCESS);
	...
	err = kernel_2.setArg(0, Buffer_In2);
	assert(err==CL_SUCCESS);
	err = kernel_2.setArg(1, vectorSize);
	assert(err==CL_SUCCESS);
	...
	err=myqueue_2.enqueueTask(kernel_2);
	assert(err==CL_SUCCESS);
