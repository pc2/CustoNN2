...
int main(void)
{
	...
	assert(err==CL_SUCCESS);
cl::CommandQueue inputmyqueue(mycontext, DeviceList[0]); 
	assert(err==CL_SUCCESS);
	...
	err = inputmyqueue.enqueueWriteBuffer(Buffer_In2, CL_FALSE, 0, sizeof(cl_float)*vectorSize, Y);
	assert(err==CL_SUCCESS);
	myqueue.finish();
	...
	const char *inputkernel_name = "InputKernel"
	...
	assert(err==CL_SUCCESS);
cl::Kernel inputkernel(program, inputkernel_name, &err);
	assert(err==CL_SUCCESS);
	...
	assert(err==CL_SUCCESS);
	err = inputkernel.setArg(0, Buffer_In2);
	assert(err==CL_SUCCESS);
	err = kernel.setArg(1, Buffer_Out);
	assert(err==CL_SUCCESS);
	err = kernel.setArg(2, vectorSize);
	assert(err==CL_SUCCESS);
err = inputkernel.setArg(2, vectorSize);
	assert(err==CL_SUCCESS);
...
err=inputmyqueue.enqueueTask(inputkernel);
	assert(err==CL_SUCCESS);	
