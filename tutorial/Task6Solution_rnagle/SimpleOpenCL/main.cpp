...
int main(void)
{
	...
	const char *kernel_name1 = "InputKernel";
	...
	cl::Kernel kernel1(program,kernel_name1,&err);
	//assert(err=CL_SUCCESS);
	...
	err = kernel1.setArg(0, Buffer_In2);
	assert(err==CL_SUCCESS);
	err = kernel1.setArg(1,vectorSize);
	assert(err==CL_SUCCESS);
	...
	err=queue2.enqueueTask(kernel1);
	assert(err==CL_SUCCESS);
	// read the output
	//////////////       Exercise 2   Step 2.8    ///////////////////
