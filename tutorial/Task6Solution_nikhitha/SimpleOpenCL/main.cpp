...
int main(void)
{
	...
	cl::CommandQueue myqueue_input(mycontext, DeviceList[0]);
	assert(err==CL_SUCCESS);
	...
	err = myqueue_input.enqueueWriteBuffer(Buffer_In2, CL_FALSE, 0, sizeof(cl_float)*vectorSize, Y);
	assert(err==CL_SUCCESS);
	myqueue.finish();
	myqueue_input.finish();

#ifndef EXERCISE1
	// create the kernel
	const char *kernel_name = "SimpleKernel";
	const char *kernel_input_name = "InputKernel";
	...
	assert(err==CL_SUCCESS);
	cl::Kernel kernel_input(program, kernel_input_name, &err);
	assert(err==CL_SUCCESS);
	...
	err = kernel_input.setArg(0, Buffer_In2);
	assert(err==CL_SUCCESS);
	err = kernel_input.setArg(1, vectorSize);
	assert(err==CL_SUCCESS);
	...
	err=myqueue_input.enqueueTask(kernel_input);
	assert(err==CL_SUCCESS);
