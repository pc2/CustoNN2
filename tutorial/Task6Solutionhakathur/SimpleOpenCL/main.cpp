...
static const cl_uint workSize = 8;
...
int main(void)
{
	...
	const char *kernal_name_input = "InputKernel";
	...
	cl::Kernel kernel_input(program, kernal_name_input, &err);
	assert(err==CL_SUCCESS);
	...
	// Launch Kernel
	//////////////       Exercise 2   Step 2.7    ///////////////////
	//err=myqueue.enqueueNDRangeKernel(kernel_input, cl::NullRange, cl::NDRange(vectorSize), cl::NDRange(workSize));
	//err=myqueue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(vectorSize), cl::NDRange(workSize));

	err=myqueue.enqueueTask(kernel_input);
	err=myqueue.enqueueTask(kernel);

	assert(err==CL_SUCCESS);
    printf("Simple kernel and kernel_inputs launched \n");
	// read the output
	//////////////       Exercise 2   Step 2.8    ///////////////////
