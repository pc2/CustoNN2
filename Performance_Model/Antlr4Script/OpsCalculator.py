'''
A python script to calculate number of MAC operations.
The big idea is to use Antrl4 lexer and parser for C language`s grammar 
and to use them to walk the generated tree by Antlr4 for the required kernel file.
The way to extract identifiers , loop limits etc is through updating different flags to identify
where we are in the tree and to grab info from this.

In this file , we consider only the operations being done at Convolution layer as MAC operations.
MAC - Multiply  and Accumulate. We consider addition and multiplication , both combined together , as one operation.
'''
...
class CPrintListener(CListener):
    ...
    def exitCompilationUnit(self, ctx):
        ...
        print("Total number of MAC ops in {} are  ".format(self.filename) +str(sum_convs))
...
def main():
    ...
    parser.add_argument('-v',action ="store_true",
                        help='Verbose mode to print more', required=False)
