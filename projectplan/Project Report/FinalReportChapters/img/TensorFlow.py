...
with tf.Graph().as_default() as graph: 
    ...
    with tf.Session() as sess:
        ...
        print("load graph")
        # We load the protobuf file from the disk and 
		# parse it to retrive the unserialized graph_drf
        with tf.gfile.FastGFile("frozen_quant.pb",'rb') as f:
            ...
            run_until = 'InceptionV1/Logits/Predictions/Softmax:0'                                
			...
			#the format for run_until is different 
			#for maxpools and concat which
			...
			or run_until.find(exception_list[2]) != -1 or  run_until.find(exception_list[3]) != -1) :
				exception_layer = 1
			...
			if (exception_layer == 0):    
				...
				inception_model,inception_model_re,inception_level,  
				operation_name_mit_extra = run_until.split("/") 
				operation_name =  operation_name_mit_extra.split(":")
				branch_level =  str("keinbranch")  
				file_path_info =  str(inception_level)+ "_" +  str(branch_level)    
				+ "_" + str(operation_name[0])  
