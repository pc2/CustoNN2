...
__kernel void MaxPool_2a_3x3_MaxPool(__global float *restrict tensor)
{
    ...
    for (int ax1 = 0; ax1 < 64; ++ax1)
    {
        for (int ax2 = 0; ax2 < 56; ++ax2)
        {
            for (int ax3 = 0; ax3 < 56; ++ax3)
            {
                ...
                write_channel_intel(maxpool_2a_out_channel, tensor[((((ax1 * 56) + ax2) * 56) + ax3)]);
            }
        }
    }
}
...
__kernel void MaxPool_3a_3x3_MaxPool(__global float *restrict tensor)
{
...
__kernel void Mixed_3b_Branch_3_MaxPool_0a_3x3_MaxPool(__global float *restrict tensor)
{
    float input0[192*28*28];
    for (int i = 0; i < 192*28*28; i++){
