...
#pragma unroll 16
                for (int rc = 0; rc < 512; ++rc)
                {
                    ...
                    float temp = fa_shiftreg[ShiftReg - 1] + (convInput[((((rc * 14) + yy) * 14) + xx)] * input1[((ff * 512) + rc)]);

#pragma unroll
                    for (unsigned j = ShiftReg - 1; j > 0; j--)
                        fa_shiftreg[j] = fa_shiftreg[j - 1];

                    fa_shiftreg[0] = temp;
                }
#pragma unroll
                for (unsigned i = 0; i < ShiftReg; i++)
                {
